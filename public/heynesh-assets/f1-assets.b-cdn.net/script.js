  ...
  const ButtonHover = {
    init() {
      ...
      document.querySelectorAll('[data-button-hover]').forEach(el => {
        ...
        let wordsA = [], wordsB = [];
        let hasPreexistingMarkup = false;

        // Check if the DOM already contains the split structure from static html
        const existingButtonMask = el.querySelector('.nav-button-mask');
        const existingLinkMask = el.querySelector('.nav-link-mask-inner');

        if (existingButtonMask) {
          mask = existingButtonMask;
          originalTarget = existingButtonMask.querySelector('.button-text:not(.clone-p)');
          cloneTarget = existingButtonMask.querySelector('.button-text.clone-p');
          if (originalTarget && cloneTarget) {
            const foundA = originalTarget.querySelectorAll('.word');
            const foundB = cloneTarget.querySelectorAll('.word');
            if (foundA.length > 0 && foundB.length > 0) {
              wordsA = foundA;
              wordsB = foundB;
              hasPreexistingMarkup = true;
            }
          }
        } else if (existingLinkMask) {
          mask = existingLinkMask;
          originalTarget = existingLinkMask.querySelector('span:not(.clone-text)');
          cloneTarget = existingLinkMask.querySelector('span.clone-text');
          if (originalTarget && cloneTarget) {
            const foundA = originalTarget.querySelectorAll('.word');
            const foundB = cloneTarget.querySelectorAll('.word');
            if (foundA.length > 0 && foundB.length > 0) {
...
const ResizeHandler = {
  ...
  // Auto-load disabled; AnimationEngine.init() will be called manually by React Client Component wrapper
  /*
