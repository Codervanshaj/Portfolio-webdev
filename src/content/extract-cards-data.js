...
for (let i = 1; i <= 7; i++) {
  ...
  let heading = headingMatch ? headingMatch[1].replace(/<[^>]*>/g, '').trim() : '';
  
  const descMatch = cardHtml.match(/class="op80"[^>]*>([\s\S]*?)<\/div>/);
  let description = descMatch ? descMatch[1].replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim() : '';
  
  // Extract images using a better regex that parses any <img ...> tag
  ...
  // Separate popup images from card images
  // Popup images are inside <div class="popup-card-wrap">
  ...
  const bottomTextMatch = cardHtml.match(/class="about-card-bottom-text"[^>]*>([\s\S]*?)<\/div>/);
  let bottomText = bottomTextMatch ? bottomTextMatch[1].replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim() : '';
  ...
  if (popupWrapIdx !== -1) {
    ...
    const popupBody = popupBodyMatch ? popupBodyMatch[1].replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim() : '';
  ...
  // Extract animations exactly
  const animAttrs = {};
  const dataAttrs = ['data-tl-from', 'data-tl-to', 'data-tl-start', 'data-tl-end', 'data-tl-trigger', 'data-tl-type', 'data-tl-once', 'data-tl-split'];
  
  // We want to capture the card's data attributes and the line's data attributes
  // Let's get the <div class="about-card" tag
